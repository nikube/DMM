...
/**
 * \file    admin/catalog.php
 * \ingroup dolimodulemanager
 * \brief   Module catalog page
 */
...
if ($action == 'addrepo' && $user->hasRight('dolimodulemanager', 'write')) {
	...
	if (!$error) {
		if (empty($module_id)) {
			// Derive module_id from repo name
			$parts = explode('/', $repo);
		...
		// Check if module_id + repo already exists
...
if ($action == 'checkupdate' && $id > 0) {
	...
	if ($result === null) {
		...
		setEventMessages('Check complete for '.$mod->module_id, null, 'mesgs');
	}
	header('Location: '.$_SERVER['PHP_SELF'].'?filter='.$filter);
	exit;
}
...
$title = $langs->trans('DMMCatalogTab');

llxHeader('', $title, '', '', 0, 0, '', '', '', 'mod-dolimodulemanager page-admin-catalog');
...
print dol_get_fiche_head($head, 'catalog', $langs->trans('DoliModuleManager'), -1, 'fa-cubes');
...
foreach ($filters as $fkey => $flabel) {
	...
	print '<a class="tab" href="'.$_SERVER['PHP_SELF'].'?filter='.$fkey.'">'.$langs->trans($flabel).'</a>';
	print '</div>';
}
print '</div>';
...
foreach ($modules as $mod) {
	...
	print '<td>'.dol_escape_htmltag($mod->github_repo).'</td>';
	...
	print '<td class="center">'.($mod->cache_last_check ? dol_print_date($mod->cache_last_check, 'dayhour') : $langs->trans('DMMNeverChecked')).'</td>';
	...
	if ($user->hasRight('dolimodulemanager', 'write')) {
		...
		print '<a class="paddingright" href="'.$_SERVER['PHP_SELF'].'?action=removemodule&token='.newToken().'&id='.$mod->id.'&filter='.$filter.'" title="'.$langs->trans('Delete').'">'.img_picto($langs->trans('Delete'), 'delete').'</a>';
	}
	print '</td>';
	print '</tr>';
}
...
if ($user->hasRight('dolimodulemanager', 'write')) {
	...
	print '<td><input type="text" name="github_repo" class="minwidth300" placeholder="owner/repository" value="'.dol_escape_htmltag(GETPOST('github_repo')).'"></td></tr>';

	print '<tr class="oddeven"><td>'.$langs->trans('DMMModuleId').' <em class="opacitymedium">(auto-detected)</em></td>';
	print '<td><input type="text" name="module_id" class="minwidth200" placeholder="auto" value="'.dol_escape_htmltag(GETPOST('module_id')).'"></td></tr>';
